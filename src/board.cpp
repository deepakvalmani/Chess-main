...
Board::Board() {
    ...
    for (int file = 0; file < 8; ++file) {      // x-axis (columns A-H)
        for (int rank = 0; rank < 8; ++rank) {  // y-axis (rows 1-8)
    ...
    // Black pieces
    grid[0][0] = std::make_unique<Rook>(false); grid[7][0] = std::make_unique<Rook>(false);
    grid[1][0] = std::make_unique<Knight>(false); grid[6][0] = std::make_unique<Knight>(false);
    grid[2][0] = std::make_unique<Bishop>(false); grid[5][0] = std::make_unique<Bishop>(false);
    grid[3][0] = std::make_unique<Queen>(false);
    grid[4][0] = std::make_unique<King>(false);
    
    for (int i = 0; i < 8; ++i) if(grid[i][0]) grid[i][0]->setPosition(i, 0);

    // White pieces
    grid[0][7] = std::make_unique<Rook>(true); grid[7][7] = std::make_unique<Rook>(true);
    grid[1][7] = std::make_unique<Knight>(true); grid[6][7] = std::make_unique<Knight>(true);
    grid[2][7] = std::make_unique<Bishop>(true); grid[5][7] = std::make_unique<Bishop>(true);
    grid[3][7] = std::make_unique<Queen>(true);
    grid[4][7] = std::make_unique<King>(true);

    for (int i = 0; i < 8; ++i) if(grid[i][7]) grid[i][7]->setPosition(i, 7);
}
